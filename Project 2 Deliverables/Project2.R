...
toDummyVarsWriters <- function(df, colName) {  
  ...
  unnested$colName <- collapseCat(unnested$colName, colName, threshold = .0009) # remove categories which are of proportion < .0001 for feature selection
...
directorBin = toDummyVars(moviesForBin, "Director", .0025)
productionBin = toDummyVars(moviesForBin, "Production", .01)
ratedBin = toDummyVars(moviesForBin, "Rated")
languageBin = toDummyVars(moviesForBin, "Language", .01)
countryBin = toDummyVars(moviesForBin, "Country", .005)
...
residDF = as.data.frame(summary(M2)$residuals)

qplot(x= dfTrain5$Budget,
      y= summary(M2)$residuals,
      data = residDF,
      main = "Residuals",
      geom = "point")