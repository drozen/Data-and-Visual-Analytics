...
#M1 = lm(price~carat+cut, diamSmall);
...
#visualize the regression   (need to use geom_line in this case instead of abline)
D<-data.frame(x=X,y=Y)
ggplot(D,aes(x=X,y=Y))+geom_line(size=2,color=I("red")) 
+ geom_point(data=diamSmall, aes(x=carat,y=price)) # overlay with original datapoints, need to use different DF 
