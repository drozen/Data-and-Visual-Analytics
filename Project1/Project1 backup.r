...
# # find unique Genres:
# #create new dataframe to separate Genres to
# GenreSep=data.frame(movieOnly$Genre)
# # separate runtime column into minutes and min
# GenreSep = separate(GenreSep, "movieOnly.Genre", c("Genre1", "Genre2", "Genre3"), ", ")
# Unique1 = unique(GenreSep$Genre1)
# Unique2 = unique(GenreSep$Genre2)
# Unique3 = unique(GenreSep$Genre3)
# 
# UniqueTotal = append(Unique1, Unique2)
# UniqueTotal = append(UniqueTotal, Unique3)
# uniqueGenres = na.omit(unique(UniqueTotal))
# uniqueGenres = uniqueGenres[uniqueGenres != "N/A"] #remove "N/A"
# # add it to the dataframe as additional columns. 
# uniqueGenres
# length(uniqueGenres)


# #convert dataframe into TM corpus
# m = list(Genre1="Genre1", Genre2="Genre2", Genre3="Genre3")
# myReader <- readTabular(mapping = m)
# myCorpus = Corpus(DataframeSource(GenreSep), readerControl = list(reader = myReader))

#myCorpus = Corpus(DataframeSource(GenreSep))
#dtm = DocumentTermMatrix(myCorpus, list(dictionary = uniqueGenres))
...
#barplot(binaryGenreVector, main="relative proportions of titles having the top 10 genres" )
...
# (explore the pairwise relationships across ImDB descriptors and Rottentomatoes descriptors(like the relationship between imdb rating and tomator user rating)  
# 
# and within ImDB descriptors as well(like between imdbrating and metascore))
...
#todo: split like earlier and adapt to Q7
...
# the above assignments.
    ...
    meanGrossPerYear = ddply(movieOnlyWithBinaryGrossBadDataRemoved,
      "Year",
      summarise,
      mean.gross = mean(Gross, na.rm = TRUE))
...
# calculate mean Gross Revenue vs. Genre

drama = dcast(grossGenreLong,
              drama~variable, 
              fun.aggregate = mean)

comedy = dcast(grossGenreLong,
               comedy~variable, 
               fun.aggregate = mean)

short = dcast(grossGenreLong,
              short~variable, 
              fun.aggregate = mean)
romance = dcast(grossGenreLong,
                romance~variable, 
                fun.aggregate = mean)
action = dcast(grossGenreLong,
               action~variable, 
               fun.aggregate = mean)
crime = dcast(grossGenreLong,
              crime~variable, 
              fun.aggregate = mean)
thriller = dcast(grossGenreLong,
                 thriller~variable, 
                 fun.aggregate = mean)
documentary = dcast(grossGenreLong,
                    documentary~variable, 
                    fun.aggregate = mean)
adventure = dcast(grossGenreLong,
                  adventure~variable, 
                  fun.aggregate = mean)
animation = dcast(grossGenreLong,
                  animation~variable, 
                  fun.aggregate = mean)
